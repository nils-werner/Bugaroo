...
<xsl:template match="dashboard-projects/entry" mode="detailed">
	...
	<li class="{status/item/@handle}"><a href="{$root}/{title/@handle}/?assignee={$member-id}"><xsl:value-of select="title" /></a> <a class="all" href="{$root}/{title/@handle}/">all issues</a>
		<ul>
...
<xsl:template match="dashboard-projects-milestones/entry">
	<li class="{status/item/@handle}"><a href="{$root}/{project/item/@handle}/?milestone={@id}&amp;assignee={$member-id}"><xsl:value-of select="title" /></a> <a  class="all" href="{$root}/{project/item/@handle}/?milestone={@id}">all issues</a>
		<ul class="details">
			<xsl:apply-templates select="/data/index-status/entry[dashboard = 'Yes']">
